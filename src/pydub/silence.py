...
def detect_silence(audio_segment, min_silence_len=1000, silence_thresh=-16, seek_step=1):
    ...
    for silence_start_i in silence_starts:
        ...
        if not continuous and silence_has_gap:
            #print([current_range_start, prev_i + min_silence_len], min_silence_len)
            silent_ranges.append([current_range_start, prev_i + min_silence_len])
            current_range_start = silence_start_i
        #import inspect;print ("karldbg %s %d" %(__file__, inspect.currentframe().f_lineno))
        #print(prev_i, silence_start_i)
        #import inspect;print ("karldbg %s %d" %(__file__, inspect.currentframe().f_lineno))
...
def detect_nonsilent(audio_segment, min_silence_len=1000, silence_thresh=-16, seek_step=1):
    ...
    for start_i, end_i in silent_ranges:
        ...
        nonsilent_ranges.append([prev_end_i, end_i])
        prev_end_i = end_i
...
def split_on_silence_with_max_len(audio_segment, min_silence_len=1000, silence_thresh=-16, keep_silence=100, seek_step=1, max_len=5000):

    not_silence_ranges = detect_nonsilent(audio_segment, min_silence_len, silence_thresh, seek_step)

    chunks = []
    for start_i, end_i in not_silence_ranges:
        if end_i - start_i < max_len:
            chunks.append(audio_segment[start_i:end_i])
        else:
